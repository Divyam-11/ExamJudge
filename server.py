# server.py
# Receives data from student monitor and sends alerts to examiner dashboard.
...
@app.route('/dashboard')
def dashboard():
    """Serves the main examiner dashboard HTML page."""
    return render_template('index.html')

@app.route('/')
def index():
    """Root URL to confirm the server is running."""
    return "Monitoring server is running. Go to /dashboard to see the examiner view."
...
def log_activity():
    ...
    print(f"Received from {student_id}: {event_type}")
...
@socketio.on('connect')
def handle_connect():
    print('Examiner dashboard connected.')
...
if __name__ == '__main__':
    # --- GET LOCAL IP AND PRINT DASHBOARD URL ---
    host_ip = get_local_ip()
    port = 5000
    print("=====================================================")
    print("          SERVER IS STARTING")
    print(f" 🖥️  Examiner Dashboard URL: http://{host_ip}:{port}/dashboard")
